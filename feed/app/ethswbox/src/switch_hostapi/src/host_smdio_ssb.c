...
static int gsw_smdio_read(const GSW_Device_t *pdev, uint16_t phy_reg)
{
	...
	ret = pdev->mdiobus_write(pdev->mdiobus_data, pdev->smdio_phy_addr, 0, SMIDO_SB_PHY_ADDR_REG, phy_reg);
	if (ret == 0) {
		ret = pdev->mdiobus_read(pdev->mdiobus_data, pdev->smdio_phy_addr, 0, SMIDO_SB_PHY_DATA_REG);
	}
...
static int gsw_smdio_write(const GSW_Device_t *pdev, uint16_t phy_reg, uint16_t phy_reg_data)
{
	...
	ret = pdev->mdiobus_write(pdev->mdiobus_data, pdev->smdio_phy_addr, 0, SMIDO_SB_PHY_ADDR_REG, phy_reg);
	if (ret == 0) {
		ret = pdev->mdiobus_write(pdev->mdiobus_data, pdev->smdio_phy_addr, 0, SMIDO_SB_PHY_DATA_REG, phy_reg_data);
	}
...
static int gsw_smdio_cont_write(const GSW_Device_t *pdev, uint16_t phy_reg, uint16_t phy_reg_data[8], uint8_t num)
{
	...
	if (ret == 0) {
		for (uint8_t i = 0; i < (num & 0xF); i++) {
			ret = pdev->mdiobus_write(pdev->mdiobus_data, pdev->smdio_phy_addr, 0, SMIDO_SB_PHY_DATA_REG, phy_reg_data[i]);
		}
	}
...
int check_registers()
{
	...
	for ( i = 0; i < SMDIO_PHY_NR; i++ )
	{
		...
		ret = int_gphy_read(host_smdio_ops.pdev, &param);
		if ((ret != 0) || (param.data == 0xffff))
			return -1;
